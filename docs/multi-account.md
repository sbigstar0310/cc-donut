...
자동 스왑은 두 단계다 (`ccd account swap`). **측정**은 네트워크 작업이라 어떤 락도 잡지
않고 하고, **설치**는 쓰기라서 저장소 락(`accounts/.lock`) 안에서 한다. 바깥을 감싸던
`.refresh.lock` 트랜잭션은 없어졌다 — 그 락은 이제 keepalive 의 단일 실행에만 쓰인다.
측정하는 사이 세상이 바뀌었을 수 있으므로, 락을 잡은 뒤 활성 계정을 다시 확인하고
(`--from` CAS) 대상 계정도 락 안에서 새로 읽는다. 락 밖에서 읽어 둔 사본은 그 사이
회전돼 **은퇴한 일회용 토큰**일 수 있기 때문이다.

```
swap(--from cur, --window key):
  0. 최근 떠난 계정 제외(§5.2) 후 후보 선정 — 락 없이 (예산이 없으면 캐시만)
...
**(3) 부분 성공한 설치는 스탬프를 거짓말하게 만든다.** *(#57 에서 해소: `live_write` 는 이제
백엔드별 결과 `(ok, failed, written)` 를 돌려주고 모든 대상이 받아야 성공이다. 아래는 당시
기록이다.)* 당시 `live_write` 는 두 백엔드 중 하나만 성공해도 참을 돌려줬다. macOS 에서 키체인 쓰기가 실패하고 파일만 성공하면 `swap_to` 는
...
### 13.9 statusline 의 spare 행: 상태는 4개다

스페어를 `ready`/`dead`/`unknown` 3개 버킷으로 분류했더니, **등록돼 있고 살아 있지만 지금
여유가 없는** 계정이 어디에도 들어가지 않았다. 세 리스트가 다 비면 마지막 분기가
"스페어 없음" 을 찍는다 — 13분 뒤 되살아날 계정을 두고 사용자에게 없다고 말한 것이다
(#15). `ccd status` 는 같은 데이터로 이걸 구분한다("No spare has room right now").

버킷을 4개로 만들고 그 문장 자체를 지웠다. `len(names) < 2` 에서 이미 early return 하니
스페어가 1개 이상이면 네 버킷이 빠짐없이 덮는다 — "없다" 고 말할 분기가 **구조적으로**
남지 않는다. 운에 맡기지 않도록 스모크가 소스 수준에서 고정한다.

행에는 %와 리셋까지 남은 시간을 함께 적는다(#16). **괄호 안의 시간은 버킷마다 다른
질문에 답한다.** 여유 있는 스페어에게 물을 것은 "지금의 압박이 언제 풀리나"이므로 결정
창(가장 많이 쓴 창)의 리셋을 적는다. 여유 없는 스페어에게 물을 것은 "언제 쓸 수 있게
되나"이고, 그 답은 **막고 있는 창들 중 마지막**이 도는 시각이다 — `has_headroom` 은 모든
창이 경계 아래일 때만 여유로 보기 때문이다(§`bin/ccd-account:985`).

처음엔 둘 다 결정 창의 리셋으로 적었고, Codex 리뷰가 반례를 냈다: 5h 100%(13분 뒤 리셋)
/ 7d 100%(6일 뒤). 13분을 적으면 **오지 않는 구원을 약속한다.** 정렬도 같은 숫자로 해야
한다 — 그러지 않으면 6일 뒤에나 쓸 계정이 2시간 뒤 쓸 계정을 앞질러 행에 오른다. 고치기
전 코드가 정확히 그랬다.

막고 있는 창 중 시각을 모르는 게 하나라도 있으면 시간을 아예 적지 않는다. 그 창도 돌아야
하므로 아는 것들만 모아 계산한 숫자는 실제보다 이르다. 리셋 시각이 이미 지났으면 역시
적지 않는다 — 캐시가 리셋보다 낡았다는 뜻이고, 그렇다면 낡은 쪽은 옆의 퍼센트다. 분은
올림한다: 내림은 남은 대기를 실제보다 짧게 말하고, 모든 대기의 마지막 1분을 `(0m)` 으로
