...
> 단어 + exit 0)이다: **등록된 모든 계정마다 — 저장소가 지금 가리키는 계정도 예외 없이 —**
> 이 계정의 것이고, 성공했고, `CANDIDATE_TTL` 보다 새롭고, 시간대가 있는 미래의 리셋 시각을
> 가진 창이 바닥인 측정이 있어야 한다. "활성 계정" 을 빼면 증명이 누가 활성인지에 달리게
> 되고, 다른 세션이 저장소를 여유 있는 계정으로 옮기는 순간 그 계정이 증명에서 빠진다.
> 활성 계정의 측정은 **벽에서의 재측정 자체**가 남긴다: `ccd-account usage` 가 라이브 토큰을
> (refresh 없이) 측정하고, 그 토큰이 등록된 계정의 것임이 확인되면 같은 값을 그 계정의
> `readings/<이름>.json` 으로도 발행한다 (`measure_active` 는 그 안으로 흡수돼 없어졌다).
> `exhausted` 자체는 읽기만 하고, 런처는 relaunch 직전에 같은 질문을 다시 한다. 측정값의
> 비교-후-교체는 계정별 락(`readings/.<이름>.lock`) 안에서 한 단계로 일어난다.
> 읽지 못한 디렉터리, 실패한 측정, 낡은 값, 락 타임아웃, 만료된 데드라인, `store-split` 정지,
> 실패했거나 확인되지 않은 스왑은 어느 것도 증명이 아니고, 그때는 기존의 "세 번 시도 후
> 멈추고 알림" 으로 간다. 틱에서는 절대 과금하지 않는다 — 벽에 실제로 부딪힌 `StopFailure`
> 에서만, 그리고 거기서 방금 성공한 재측정이 있을 때만.
>
...
계정이 여러 개인 사용자가 한 계정의 쿼타를 소진했을 때, **여유가 있는 다른 Claude 계정으로 세션 안에서 이동**한다. 지금 계정이 100% 가 되면 틱에서 옮기고, 벽에 먼저 부딪히면 `StopFailure` 가 뒷받침한다 — 벽에서는 **그 자리에서 방금 성공한 측정**만 근거가 된다 (재측정이 실패하면 아무 일도 하지 않는다). 갈 구독이 없다는 것이 **증명**되고(등록된 **모든** 계정이 — 지금 쓰는 계정 포함 — 방금 측정돼 100%, 또는 등록된 계정이 없음), 키가 저장돼 있고, `ccd setup --auto` 로 허용돼 있으면 런처를 통해 OpenRouter 백본으로 relaunch 한다. 그중 하나라도 아니면 — 스왑이 그냥 실패한 경우를 포함해 — 세 번까지 시도한 뒤 **멈추고 한 줄로 알린다**; 과금의 근거는 증명뿐이다. 거기서 구독으로 **돌아오는 길**도 런처가 맡는다.
...
- **잡는 것과 계속 쥐고 있는 것은 다르다** (#96). 랩탑이 자거나 훅이 멈추거나 키체인이
  stale 창을 넘겨 답하면 그 사이 Claude Code 가 락을 가져가 **떠나는 계정을 refresh 하기
  시작한다** — 그 상태로 멈추기 전의 읽기에서 설치하면 #67 이 그대로 재현된다. 그래서
  `claude_code_locks` 는 리스(`Lease`)를 내주고, writer 가 **백업 직전과 모든 라이브 저장소
  쓰기 직전**(설치·롤백·같은 계정 정렬 쓰기)에 아직 우리 것인지 묻는다. 끊겼으면 기한이
  지났을 때와 같이 **아무것도 쓰지 않고** 실패한다. mtime 갱신이 **실패한** 락도 끊긴 것으로
  본다 — 갱신할 수 없는 락은 우리 밑에서 stale 이 되어 언제든 넘어간다.
- 기다림은 호출자의 기한 안에서만 (`--deadline`: 프롬프트 틱 3초, StopFailure 60초,
  `ccd account use` 5초). 넘기면 **아무것도 쓰지 않고** 실패하고, 기존의 3회 시도 후 알림이
  처리한다. 기한은 **모든 취득 경로의 매 회전에서** 본다 — 기다린 락뿐 아니라 비어 있어
  `mkdir` 이 바로 성공한 락, 사이에 사라진 락, stale 이라 인계하는 락까지. 기한이 지난 뒤
  풀린 락은 가져가지 않고, ccd 자신의 저장소 락에도 0.5초의 여유를 따로 주지 않는다.
...
| 상황 | 동작 |
|---|---|
| 등록된 모든 계정이 100% (지금 쓰는 계정 포함, 측정으로 증명됨) | 키 + `--auto` 옵트인 + 런처가 있으면 `to_fallback`. 하나라도 없으면 멈추고 알림 |
| 등록된 계정 없음 (디렉터리를 읽어서 확인됨) | 위와 같다 — 등록된 계정이 없다는 것 자체가 증명이다 |
| 벽에서의 재측정 실패 (429·타임아웃 등) | **아무 일도 하지 않는다.** 캐시에 남은 옛 측정값은 표시용일 뿐 근거가 아니다 |
| rate_limit 으로 턴이 죽었는데, 방금 측정한 지금 계정에 여유가 있음 | 스왑도 과금도 없이 **턴을 한 번 깨워 재시도**시킨다 (틱이 방금 스왑했고 요청은 옛 자격증명으로 나간 경우, 다른 세션이 저장소를 옮긴 경우, 일시적 스로틀). 세션당 `PROBE_BACKOFF_TTL`(5분)에 한 번; 그 안에 또 죽으면 깨우지 않고 note 를 남긴다 |
...
**#74 개정.** 복귀는 창 하나가 아니라 **모든 창**을 본다 — 리딩이 보고하는, 아직 리셋되지 않은 창이 전부 `bin/spent-at` 미만일 때만 `to_subscription` 을 무장하고, 리딩이 없거나 너무 낡았으면 무장하지 않는다 (탈출 경로는 원래 `has_headroom` 을 지나므로 그대로다). 5시간 창만 리셋되고 주간 창이 아직 100% 인데 돌아가면 relaunch 를 두 번 쓰고 몇 분 만에 같은 벽으로 되돌아올 뿐이며, `HOP_RESET_SECONDS` 가 짧으면 런처의 `visited` 가드가 그 세션을 거기 가둔다.
