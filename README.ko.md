...
구독이니까요. 쿼타가 초기화됐다고 알아서 원래 계정으로 되돌리지는 않습니다. 지금 쓰는
계정이 다시 바닥나 갈 때, 여유가 있는 구독으로 옮길 뿐입니다. 그게 처음 쓰던 계정인
경우가 많습니다.

두 구독의 쿼타가 동시에 바닥나는 일은 드물어서, 대부분은 계정 전환만으로 계속 쓸 수
있습니다. OpenRouter는 본인 키를 쓰는 유료 최후 수단입니다. cc-donut이 알아서 그쪽으로
넘기는 것은 세 가지가 모두 맞을 때뿐입니다. 세션을 받아 줄 구독이 없고(등록된 예비
구독이 없거나, 전부 바닥난 것이 측정으로 확인됨), 키를 저장해 두었고, `ccd setup
--auto`로 허용해 둔 경우입니다. 그 밖에는 멈추고 그 사실을 알려 줍니다. 전환이 그냥
실패한 경우에도 세 번까지 시도한 뒤 멈출 뿐, 그것을 과금의 이유로 삼지 않습니다.
직접 넘어가려면 언제든 `ccd -c` 를 치면 됩니다.
...
ccd setup           # statusline과 훅 배선. 전환에는 이 이상이 필요 없습니다
```

구독 두 개를 쓰는 세팅은 이게 전부입니다. 구독 사이 전환은 **실행 중인 세션 안에서**
일어나므로 깔 런처도, 재시작도 없습니다. OpenRouter(`ccd key`)는 선택 사항이고,
...
자동       칠 것 없음                 세션이 그 자리에서 다른 구독으로
                                      옮겨갑니다

수동       !ccd account use           한 줄 치고 그대로 계속
```

둘 다 지금 쓰고 있는 그 세션을 옮깁니다. `!`를 붙이면 Claude Code를 벗어나지 않고 셸
명령을 실행할 수 있고, 자동 전환은 쿼타 조회가 "다 썼다"고 말하는 순간 ccd가 같은 일을
대신 해주는 것입니다. 런처도, 재실행도, 끝낼 것도 없습니다. 조회보다 한도가 먼저
도착하면 ccd가 계정을 바꾼 뒤 멈춰 선 턴을 직접 깨웁니다.

뒤늦게 따라오는 것은 표시뿐입니다. Claude Code는 모델 목록과 Fable 사용 가능 여부,
한도, `/status` 신원을 시작할 때 한 번 읽어 들고 있어서, 다음 실행 전까지는 떠나온 계정
기준으로 보입니다. ccd가 그 캐시를 `/login`과 똑같이 비우므로 그 이상 남지는 않습니다.
...
**5시간 쿼타와 7일 쿼타가 모두** 남아 있는 계정만 후보가 됩니다. 어느 한쪽이라도 100%면
그 계정은 바닥난 것으로 봅니다. 그 아래로 따로 남겨 두는 여유분은 없습니다. 99%인 예비
계정도 갈 곳이고, 세션을 옮기는 시점도 지금 쓰는 계정이 100%가 됐을 때입니다.
...
● claude:personal │ spare work 100% (13m)      # 소진됐지만 기다릴 만함
```
...
<details>
<summary><b>유료 전환: OpenRouter, 아무것도 안 쳐도 되는 방식</b></summary>

내 구독 사이의 전환은 여기 해당하지 않습니다 — 깔 것이 없습니다. 이 항목은 등록한 구독을
전부 쓴 뒤 세션을 OpenRouter로 옮기고 되돌리는 마지막 수단에 대한 것입니다.

```sh
ccd setup --auto
```

과금을 승인하고, 런처를 `~/.claude/ccd/bin/claude`에 설치하고, 셸이 이 런처를 먼저
찾도록 셸 초기화 파일에 한 줄을 추가할지 묻습니다.
...
코드를 확인합니다. 등록한 구독을 전부 쓰면 OpenRouter에서 대화를 자동으로 이어가고,
쿼타가 초기화되면 구독으로 돌아갑니다. 돌아갈 때는 리셋된 창 하나가 아니라 리딩이
보고하는 모든 창을 봅니다 — 5시간 창만 돌아오고 주간 창이 아직 100%면 몇 분 만에 같은
벽에 다시 부딪히기 때문입니다.

자동 전환의 조건과 제약:

- **추측이 아니라 확인된 경우에만 과금합니다.** rate-limit 에러, 키, 이 승인에 더해,
  등록된 계정 전부가 — 지금 쓰는 계정까지 포함해 — 방금 성공적으로 측정됐고, 그 결과가
  100%이며 리셋 시각이 아직 미래여야 합니다(등록된 계정이 아예 없어도 됩니다). ccd에서
  "바닥"은 어디서나 100%를 뜻합니다. 지금 쓰는 계정이 100%가 되면 옮기고, 예비 구독은
  100%가 되기 전까지는 갈 곳이며(99%도 갈 곳입니다), 100%에 못 미친 계정이 하나라도
  있으면 과금하지 않습니다. 측정하지 못한 구독, 바쁜 저장소, 실패한 전환은 어느 것도
  근거가 되지 못하고, 그럴 때는 과금 대신 안내 한 줄이 남습니다.
- **런처가 실행 중이어야 합니다.** 승인은 켜져 있는데 세션을 런처로 시작하지 않았다면
  과금하지도, 된 척하지도 않습니다. 그 사실을 알려 주고, statusline이 미리 경고합니다.
  갈 곳 없이 세션을 끝내는 일은 없습니다.
- **진행 중이던 턴은 유실됩니다.** 실패한 턴 뒤에 전환이 일어나므로 마지막 프롬프트는
  다시 보내야 합니다. (구독 사이의 무료 전환은 그 자리에서 일어나고 턴을 직접 깨우므로
  유실되지 않습니다.)
...
전환에는 목적지가 둘 있고, 둘의 값이 다릅니다.

- **등록된 다른 구독.** 무료이고, 세션 안에서 일어납니다. ccd가 쓰는 자격증명이 바로
  다음 요청이 읽는 그 자격증명이라서, 재실행도 유실도 없습니다. 런처도 PATH 줄도
  재시작도 필요 없습니다.
- **OpenRouter, 본인 API 키.** 과금됩니다. 등록된 구독을 전부 쓴 뒤에만, 그리고 키를
  저장해 두고 `ccd setup --auto`로 허용해 둔 경우에만 도달합니다. 런처가 필요한 유일한
  전환이기도 합니다 — 세션이 OpenRouter 위에 있는 동안 백본은 환경 변수라서,
  거기서 나오려면 재실행이 필요합니다.

`ccd setup --auto`가 유료 전환을 승인하고 그 런처를 깝니다. 키를 저장해 둔 것만으로는
부족합니다. 그 승인 없이 모든 구독이 바닥나면 세션은 유료 백본으로 넘어가지 않고 그
자리에서 끝납니다. `ccd doctor`가 둘을 따로 보고하고, `ccd setup --no-auto`는 런처와
함께 이 승인도 거둬들입니다.

자동 전환이 작동하지 않을 때는 문서에 안내된 수동 전환 방법을 그대로 쓸 수 있습니다.
`ccd setup --no-auto`로 유료 전환을 끄고 `ccd uninstall`로 제거합니다. ccd가 만들지 않은
...
등록한 구독의 쿼타가 모두 소진됐을 때, 키를 저장해 두고 `ccd setup --auto`로 허용해
둔 경우에만 OpenRouter로 전환합니다. 본인 키를 사용하고 토큰 사용량에 따라 과금되므로,
필요해지기 전에 설정해 두는 편이 좋습니다.
...
| `ccd account use` | 여유 있는 계정으로 전환. 핸드오프가 고르는 것과 같은 계정입니다. 세션 안에서 `!ccd account use`로 하면 재시작 없이 그 세션에서 그대로 이어집니다 |
| `ccd account use <name>` | 그 계정으로 전환 |
| `ccd account rm <name>` | 계정 제거 |
| `ccd setup` | 구독 사이 전환에 필요한 전체 설치 (statusline, 훅, `ccd` 자체) |
| `ccd setup --auto` | 구독을 다 쓴 뒤 유료 OpenRouter 전환까지 허용하고 그 런처를 설치 |
| `ccd setup --no-auto` | 그 런처와 PATH 줄을 제거 |
