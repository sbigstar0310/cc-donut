...
- **세 조건이 모두 충족돼야 작동합니다.** 런처가 실행 중이고, 쿼타 조회 결과가
  rate-limit 에러와 일치하며, 키와 이 승인이 있어야 합니다. 하나라도 충족되지 않으면
...
- **등록된 다른 구독.** 무료이고, 세션 안에서 일어납니다. ccd가 쓰는 자격증명이 바로
  ...
  재시작도 필요 없습니다.
- **OpenRouter, 본인 API 키.** 과금됩니다. 등록된 구독을 전부 쓴 뒤에만 도달하고, 런처가
  필요한 유일한 전환입니다 — 세션이 OpenRouter 위에 있는 동안 백본은 환경 변수라서,
  거기서 나오려면 재실행이 필요합니다.
