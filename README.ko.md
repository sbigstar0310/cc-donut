...
있습니다. 등록한 구독이 모두 바닥나면 세 번까지 시도한 뒤 멈추고, 그 사실을 알려
줍니다. OpenRouter는 본인 키를 쓰는 유료 최후 수단이라 넘어갈 때는 항상 `ccd -c` 를
직접 칩니다. 자동으로 되는 것은 거기서 구독으로 돌아오는 길뿐입니다.
...
<details>
<summary><b>OpenRouter에서 저절로 돌아오기</b></summary>

ccd는 대화를 OpenRouter로 **옮겨주지 않습니다.** 과금되는 일이라 그건 사용자가 직접 치는
명령(`ccd -c`)입니다. ccd가 대신 해주는 것은 *돌아오는* 쪽입니다 — 예비 구독에 여유가
생기거나 창이 초기화되면, 아무것도 치지 않아도 구독으로 돌아옵니다.

```sh
ccd setup --auto
```

런처를 `~/.claude/ccd/bin/claude`에 설치하고, 셸이 이 런처를 먼저 찾도록 셸 초기화 파일에
한 줄을 추가할지 묻습니다.
...
그다음부터는 평소처럼 `claude`로 시작하면 됩니다. 그 아래에서 시작한 `ccd -c` 실행은
스스로 구독으로 돌아옵니다.

조건과 제약:

- **돌아오는 데는 런처가 필요합니다.** 세션이 OpenRouter 위에 있는 동안 백본은 환경
  변수라서, 거기서 나오려면 재실행이 필요합니다. 런처가 없으면 세션은 그 자리에
  머무르고, 손으로 돌아오는 방법을 알려줍니다.
...
- **내 구독 사이.** 자동, 무료, 세션 안에서. ccd가 쓰는 자격증명이 바로 다음 요청이 읽는
  그 자격증명이라서 재실행도 유실도 없습니다. 런처도 PATH 줄도 재시작도 필요 없습니다.
- **OpenRouter로 나가기.** 자동이 아닙니다. 갈 수 있는 예비 계정이 없으면 ccd는 멈추고,
  이유를 한 줄 남기고, `ccd -c` 를 알려줍니다 — 본인 키와 본인 청구서로, 직접 칩니다.
- **OpenRouter에서 돌아오기.** 런처가 깔려 있으면 자동입니다.

`ccd setup --no-auto`는 런처와 PATH 줄을 제거하고, `ccd uninstall`은 전부 제거합니다.
ccd가 만들지 않은 `~/.claude/ccd/bin/claude` 파일이나 ccd가 추가하지 않은 PATH 줄은
건드리지 않습니다.
...
| `ccd setup --auto` | `ccd -c` 실행이 저절로 구독으로 돌아오게 하는 런처를 설치 |
